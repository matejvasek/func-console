...
describe('AsyncQueue', () => {
  ...
  it('matches one enqueued value to one waiting consumer', async () => {
    ...
    // Exactly one consumer resolves with the value
    ...
    expect(resolved.promise).toBe('p1'); // First waiter gets the value (fairness)

    // Other consumers still pending (closing triggers rejection)
    queue.close();
