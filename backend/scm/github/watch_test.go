...
// that consumes the watcher channel. The first tickPoll() triggers an HTTP
// request and tries to send the result via the channel. The second tickPoll()
// blocks because the watch loop is stuck waiting for the consumer to accept
// the first result (unless results are unchanged, since we suppress duplicates).
//
// Mitigation: Call `go tickPoll()` instead, on a separate goroutine.
//
// Pitfall 2: Async timing when changing handler behavior between ticks.
// Example: `tickPoll(); prop = x; tickPoll()`. Both HTTP calls may see
// `prop == x`, not just the second, because the first call is still in flight.
//
